...
Click chuột sang chỗ khác khi đang gõ dở (chưa Space/Enter) không commit bừa: phần gõ dở được giữ lại, quay về đúng ô nhập thì gõ tiếp; sang ô nhập khác thì bỏ.
...
* **Build (MSYS2 MinGW-w64)**: `pacman -S mingw-w64-x86_64-gcc mingw-w64-x86_64-glib2 make`, rồi `make -f Makefile.win`. Đóng bộ cài (cần Inno Setup): `make -f Makefile.win setup VERSION=0.5.1` (version phải là `X.Y.Z`, khớp `windows/version.h`).
