...
install -Dm755 "$BUILD_DIR/gotiengviet-demo" "$DESTDIR/usr/local/bin/gotiengviet-demo"
...
mkdir -p "$DESTDIR/usr/local/bin" "$DESTDIR/usr/share/applications" "$DESTDIR/etc/xdg/autostart"
ln -sfn /usr/libexec/ibus-setup-gotiengviet "$DESTDIR/usr/local/bin/gotiengviet"
